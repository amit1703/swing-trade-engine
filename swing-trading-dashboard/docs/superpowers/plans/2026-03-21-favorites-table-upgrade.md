# Favorites Page Table Upgrade Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace card-style rows in FavoritesPage with a compact sortable table (TICKER | TYPE | DIST | SCR | ENTRY | SL | R:R | TV), one row per setup, with a single sort state.

**Architecture:** Full rewrite of `FavoritesPage.jsx`. Pure helpers `buildRows()` and `sortRows()` produce a flat sorted list. `SortHeader` renders clickable column headers. `FavRow` renders one data row. Main component manages one sort state via `useState`. No backend changes.
...
| File | What to know |
|------|-------------|
| `frontend/src/components/FavoritesPage.jsx` | Full rewrite. Current: card rows with sub-rows. Props interface unchanged. |
| `frontend/src/App.jsx` | Passes `favorites`, `onToggleFavorite`, `allSetups`, `watchlistItems`, `selectedTicker`, `onSelectTicker`, `livePrices`. Props interface must not change. |

**Data fields available:**

`allSetups` items: `ticker`, `setup_type`, `entry`, `stop_loss`, `rr`, `setup_score`, `atr`, `distance_pct`, `rs_blue_dot`

`watchlistItems` items: `ticker`, `watchlist_source` (`'RES_BREAKOUT'` or `'PULLBACK'`), `entry`, `stop_loss`, `rr`, `setup_score`, `atr`, `distance_pct`, `rs_blue_dot`

`distance_pct` is stored as a percentage value (e.g. `1.2` = 1.2%) — do not multiply by 100.

**Zero guards:** Engine2-sourced items have `rr = 0`, `stop_loss = 0`, `setup_score = 0`. Treat `=== 0` same as null → display `—`.
...
| File | Action |
|------|--------|
| `frontend/src/components/FavoritesPage.jsx` | Full rewrite |

---

## Task 1: Rewrite FavoritesPage.jsx

**Files:**
- Modify: `frontend/src/components/FavoritesPage.jsx` (full replacement)

- [ ] **Step 1: Read the current file**

Read `frontend/src/components/FavoritesPage.jsx` in full so you understand what you are replacing.
...
```bash
cd "C:\Users\1\OneDrive\Desktop\claudeSkillsTest\swing-trading-dashboard\frontend" && npx vite build 2>&1 | tail -8
```

Expected: `✓ built in` with no errors. Chunk size warnings are fine.

- [ ] **Step 4: Verify no old amber values remain**

```bash
grep -n "245,166,35\|F5A623" "C:\Users\1\OneDrive\Desktop\claudeSkillsTest\swing-trading-dashboard\frontend\src\components\FavoritesPage.jsx"
```
...
git add frontend/src/components/FavoritesPage.jsx
git commit -m "feat: favorites page sortable table with type/dist/score/entry/sl/rr columns"
```
...
- [ ] Favorites page shows a table (not cards)
- [ ] TICKER | TYPE | DIST | SCR | ENTRY | SL | R:R | TV columns visible
- [ ] TYPE badge shows VCP / PB / BRK / WL↓ / WL↑ etc with correct colors
- [ ] DIST and SCR headers are clickable and sort the table
- [ ] "Not in scan" tickers appear at the bottom with TYPE cell showing "not in scan"
- [ ] ★ button removes the ticker from favorites
- [ ] Selected ticker row highlights in cyan
- [ ] No amber/yellow colors anywhere on the page
